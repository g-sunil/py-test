...
def main():
    ...
    for file_path in partials:
        ...
        try:
            ...
            span_list = parsed_doc.getroot().findall(".//div[@class='modal-header']/span")
        except:
            pass
        for span in span_list:
            if not ('j-help' in span.attrib and span.attrib['j-help']):
                html_dict[file_name].add(etree.tostring(span).strip())
...
def write_to_file(**result_data):
    ...
    with open(os.getcwd() + os.sep + 'jhelp-report.html', 'w') as file_buffer:
